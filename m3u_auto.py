...
优化版本：修复bug + 自动学习黑名单 + 4K分辨率严格卡控 + 港台-11放行 + 合并输出
业务逻辑完全不变
"""
...
def load_auto_blacklist():
    ...
    if os.path.exists(AUTO_BLACKLIST_FILE):
        try:
            ...
            pass
    return {"urls": [], "domains": [], "patterns": []}

def save_auto_blacklist(blacklist):
    """保存自动学习黑名单"""
    try:
        blacklist["urls"] = list(dict.fromkeys(blacklist["urls"]))
        blacklist["domains"] = list(dict.fromkeys(blacklist["domains"]))
        blacklist["patterns"] = list(dict.fromkeys(blacklist["patterns"]))
        
        if len(blacklist["urls"]) > 5000:
            blacklist["urls"] = blacklist["urls"][-5000:]
        if len(blacklist["domains"]) > 1000:
            blacklist["domains"] = blacklist["domains"][-1000:]
        if len(blacklist["patterns"]) > 500:
            blacklist["patterns"] = blacklist["patterns"][-500:]
...
def add_to_auto_blacklist(url, fail_reason=""):
    ...
    try:
        ...
        if domain and not is_private_ip(domain):
            if domain not in blacklist["domains"]:
                blacklist["domains"].append(domain)
                updated = True
    except:
        pass
    ...
    try:
        ...
        if len(path_parts) >= 2:
            ...
            if pattern and pattern not in blacklist["patterns"]:
                blacklist["patterns"].append(pattern)
                updated = True
    except:
        pass
    
    if updated:
        save_auto_blacklist(blacklist)
...
def is_in_auto_blacklist(url):
    ...
    try:
        ...
        if len(path_parts) >= 2:
            pattern = "/" + "/".join(path_parts[:2])
            if pattern in blacklist["patterns"]:
                return True
    except:
        pass
...
print(f"📚 自动学习黑名单已加载: {len(auto_blacklist['urls'])} URL, "
      f"{len(auto_blacklist['domains'])} 域名, {len(auto_blacklist['patterns'])} 路径模式")
...
def step2_parse_and_evict_dead_links(lives_list):
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        ...
        for idx, f in enumerate(as_completed(futures), 1):
            ...
            print(f"   [Step2-订阅拉取] 已完成 {idx}/{total_sources} 个源，"
                  f"新增 {len(result)} 条，累计 {len(raw_channels)} 条")
    ...
    def check_alive_basic(item):
        ...
        try:
            with requests.get(
                ...
                if r.status_code in [200, 206]:
                    try:
                        ...
                        if is_hk:
                            ...
                            if not is_hk:
                                add_to_auto_blacklist(item["url"], "Step2-首包读取失败")
                            return None
                ...
                if not is_hk:
                    add_to_auto_blacklist(item["url"], f"Step2-HTTP{r.status_code}")
            ...
            if not is_hk:
                add_to_auto_blacklist(item["url"], f"Step2-{err_name}")
    ...
    with ThreadPoolExecutor(max_workers=20) as check_executor:
        ...
        for f in as_completed(futures):
            ...
            if checked_count % 100 == 0 or checked_count == total_check:
                print(f"   [Step2-活性检测] 已检测 {checked_count}/{total_check} 条，"
                      f"存活 {alive_count} 条，淘汰 {checked_count - alive_count} 条")

    print(f"✅ Step 2 完成：通过初筛活链共 {len(survived_channels)} 条。")
    return survived_channels

def step3_geo_ip_classify(channels):
    """Step 3: 检测 IP 归属地，划分国内组与港台组"""
        ...
        if is_hk_tw:
            ...
            if ip:
                ...
                if country != "CN": 
                    ...
                    add_to_auto_blacklist(ch["url"], f"Step3-非中国IP({country})")
                    continue
...
def step5_process_hktw_names(hktw_group):
    ...
    for item in hktw_group:
        ...
        name = re.sub(r'[_#\-\s\t｜|]', '', name).upper()
        name = name.replace("雙語", "").replace("双语", "").replace("高清", "").replace("FHD", "").replace("HD", "")
        name = name.replace("4GTV", "").replace("备", "").replace("TVB功夫台", "TVB亚洲武俠").replace("AMC电影台", "AMC电影")
        name = convert_t2s(name)
    ...
    try:
        with requests.get(url, headers=PLAYER_HEADERS, 
            ...
            if r.status_code not in [200, 206]:
                ...
                if not is_hktw:
                    add_to_auto_blacklist(url, f"Step6-HTTP{r.status_code}")
                return False
            ...
            if duration <= 0:
                if not is_hktw:
                    add_to_auto_blacklist(url, "Step6-测速时长为0")
                return False
            ...
            if max_jitter > max_jitter_dead or avg_speed < min_speed_dead:
                ...
                if not is_hktw:
                    add_to_auto_blacklist(url, f"Step6-生死线未达标(速度:{avg_speed/1024:.1f}KB/s)")
                return False
            ...
            if is_4k_url:
                if max_jitter > max_jitter_4k or avg_speed < min_speed_4k:
                    ...
                    if not is_hktw:
                        add_to_auto_blacklist(url, f"Step6-4K规格未达标")
                    return False
                return True
            ...
            if max_jitter > max_jitter_normal or avg_speed < min_speed_normal:
                ...
                if not is_hktw:
                    add_to_auto_blacklist(url, f"Step6-稳定性未达标(速度:{avg_speed/1024:.1f}KB/s)")
                return False
        ...
        if not is_hktw:
            add_to_auto_blacklist(url, f"Step6-异常({type(e).__name__})")
        return False

def step7_8_ffmpeg_pipeline_audit(item, cfg, is_hktw):
    """Step 7/8: 完全配置驱动的FFmpeg深度审计"""
    url = item["url"]
    ...
    try:
        ...
        if res.returncode != 0:
            # 港台组：-11(SIGSEGV)放行，FFmpeg兼容性问题，VLC验证可播
            ...
            if not is_hktw:
                add_to_auto_blacklist(url, f"Step8-FFmpeg返回码{res.returncode}")
            return False
        ...
        if not video_line:
            ...
            if not is_hktw:
                add_to_auto_blacklist(url, "Step7-无法解析Video元数据")
            return False
        ...
        # 4K分类使用严格的4K分辨率标准（2160p）
        if category == "4K超清":
            MIN_HEIGHT_4K = 2160
            if height < MIN_HEIGHT_4K:
                register_discard(7, f"归类为4K但分辨率仅{height}p（要求≥2160p）", url, is_hktw)
                if not is_hktw:
                    add_to_auto_blacklist(url, f"Step7-伪4K分辨率{height}p")
                return False
        else:
            ...
            if height < min_height:
                ...
                if not is_hktw:
                    add_to_auto_blacklist(url, f"Step7-分辨率{height}p")
                return False
        ...
        if speed_all and not allow_low_ratio:
            try:
                ...
                if speed_val < min_speed_ratio:
                    ...
                    if not is_hktw:
                        add_to_auto_blacklist(url, f"Step8-解码速率{speed_val}x")
                    return False
            except: 
                pass
        ...
        if strict_frame_check:
            if "frame=0" in stderr_output or "frame= " not in stderr_output:
                ...
                if not is_hktw:
                    add_to_auto_blacklist(url, "Step8-黑屏无帧")
                return False
        ...
        if strict_zombie_check:
            ...
            for kw, desc in zombie_keywords.items():
                if kw in stderr_output:
                    ...
                    if not is_hktw:
                        add_to_auto_blacklist(url, f"Step8-{desc}")
                    return False
        ...
        if crop_lines and width > 0 and height > 0:
            ...
            if (width - crop_w) > max_border or (height - crop_h) > max_border:
                ...
                if not is_hktw:
                    add_to_auto_blacklist(url, f"Step8-黑边过大")
                return False
        ...
        if not is_hktw:
            add_to_auto_blacklist(url, "Step8-FFmpeg超时")
        return False
    except Exception as e:
        register_discard(8, f"FFmpeg执行异常 ({type(e).__name__})", url, is_hktw)
        if not is_hktw:
            add_to_auto_blacklist(url, f"Step8-异常({type(e).__name__})")
        return False

def step9_generate_all_and_upload(domestic_list, hktw_list):
    """Step 9: 生成合并 live.m3u（央视频道 → 卫视频道 → 港台频道 → 4K超清）并上传"""
...
def main():
    ...
    if domestic_cleaned:
        with ThreadPoolExecutor(max_workers=MAX_WORKERS) as dom_executor:
            futures = {dom_executor.submit(run_quality_pipeline, item, False): item 
                      for item in domestic_cleaned}
            for f in as_completed(futures):
                res = f.result()
                if res:
    ...
    if hktw_cleaned:
        with ThreadPoolExecutor(max_workers=MAX_WORKERS) as hk_executor:
            futures = {hk_executor.submit(run_quality_pipeline, item, True): item 
                      for item in hktw_cleaned}
            for f in as_completed(futures):
                res = f.result()
                if res:
                    final_hktw_list.append(res)

    print(f"📊 检测结束：国内组通过卡点共 {len(final_domestic_list)} 条，"
          f"港台组通过卡点共 {len(final_hktw_list)} 条。")

    # 合并输出并上传
    step9_generate_all_and_upload(final_domestic_list, final_hktw_list)

    print("\n📝 正在将全量各拦截点被斩断的【国内组】无用直播源导出至 discard_report.txt ...")
    ...
    auto_bl = load_auto_blacklist()
    print(f"\n📚 自动学习黑名单统计: {len(auto_bl['urls'])} URL, "
          f"{len(auto_bl['domains'])} 域名, {len(auto_bl['patterns'])} 路径模式")
